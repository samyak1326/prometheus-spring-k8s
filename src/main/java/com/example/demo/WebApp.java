...
////    private final Histogram.Builder duration;
//
//    public WebApp(MeterRegistry registry) {
//        // Create a Counter using Micrometer's MeterRegistry
//        this.counter = Counter.builder("total_requests")
//                .description("Total number of requests to /Hello endpoint")
//                .register(registry);
//
//        this.delayTimer = Timer.builder("post_request_delay")
//                .description("Time taken processing POST requests")
//                .register(registry);
//
////        this.duration = Histogram.build().name("name").help("somehlep").unit("seconds").labelNames("Method","path", "status_code").register();
//
//
//
...
//    @PostMapping("/PostEndpoint")
//    public String tryDelay(@RequestBody JsonNode requestBody){
//        String keyValue = requestBody.has("key") ? requestBody.get("key").asText() : "Key not found";
//        System.out.println("POST request received with body: " + keyValue);
//        try{
//            long start = System.currentTimeMillis();
//
//            Thread.sleep(Integer.parseInt(keyValue));
//            long end = System.currentTimeMillis();
//            long delay = end - start ;
//
//            System.out.println("POST request - delay of " + delay + " ms");
